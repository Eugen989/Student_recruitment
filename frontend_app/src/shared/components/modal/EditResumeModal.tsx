import type { EditResume } from "../../../features/profile/profileTypes.type.ts";
import { useForm } from "react-hook-form";
import { createPortal } from "react-dom";
import { ModalContainer } from "../../../shared/components/modal/ModalContainer.tsx";
import { TextareaForm } from "../../../shared/components/TextareaForm/TextareaForm.tsx";
...
export const EditResumeModal = ({
    ...
    // Загрузка данных резюме и справочников
    useEffect(() => {
        if (isOpen && resumeId !== null) {
            // Загрузка справочников (тегов и проектов)
            setTags([
                { id: 1, name: "Frontend" },
                { id: 2, name: "Backend" },
                { id: 3, name: "Fullstack" },
            ...
            // Загрузка данных резюме (заглушка)
            const fetchResumeData = async () => {
                try {
                    ...
                    // Заглушка данных
                    const resumeData: EditResume = {
                        id: resumeId,
                        salary: 80000,
                        description: "Опытный фронтенд-разработчик с 5-летним стажем, специализирующийся на React и TypeScript. Ищу интересные проекты в сфере веб-разработки.",
                        tegsId: [1, 4],
                        projectsId: [1, 3]
                    };

                    reset(resumeData);
                    setSelectedTags(resumeData.tegsId);
                    setSelectedProjects(resumeData.projectsId);
                } catch (error) {
    ...
    }, [isOpen, resumeId, reset, onClose]);

    // Синхронизация выбранных тегов/проектов с формой
    useEffect(() => {
            ...
            >
                ...
                <form onSubmit={handleSubmit(onSubmit)} className="space-y-5">
                    ...
                    <div className="flex justify-end space-x-3 pt-2">
                        ...
                        <button
                            type="submit"
                            className="px-5 py-2 bg-purple-20 text-white font-medium rounded-xl hover:bg-purple-30 transition-colors"
                        >
                            Сохранить изменения
                        </button>
                    </div>
